...
export default function Home() {
  ...
  return <><Header /><main>
    ...
    <section id="destinations" className="scroll-mt-20 py-14 sm:py-20 px-4 sm:px-6 surface-sand"><div className="max-w-6xl mx-auto"><motion.div {...sectionReveal} className="grid md:grid-cols-[1fr_auto] md:items-end gap-5 mb-10 md:mb-12"><div><p className="text-[#84684b] text-sm font-bold">{home.toursKicker}</p><h2 className="text-3xl lg:text-4xl font-bold text-[#2b312d] leading-tight mt-3">{home.toursTitle}</h2></div><p className="text-sm leading-6 text-slate-700 md:max-w-xs md:text-right">{home.toursText}</p></motion.div><div className="grid sm:grid-cols-2 lg:grid-cols-4 gap-5 md:gap-6">{tours.map((tour, index) => <TourCard key={tour.slug} tour={tour} index={index} />)}</div></div></section>
